...
class SOAP(optim.Optimizer):
    ...
    def project(self, grad, state, merge_dims=False, max_precond_dim=10000):
        ...
        for mat in state['Q']:
            if len(mat) > 0:
                # Ensure both tensors are of the same dtype to avoid dtype mismatch
                mat = mat.to(grad.dtype)
        ...
        if grad.dim() == 1:
            ...
            if merge_dims:
                ...
                for idx, sh in enumerate(grad.shape):
                    ...
                    # Ensure both tensors are of the same dtype to avoid dtype mismatch
                        state['GG'][idx] = state['GG'][idx].float()  
                        outer_product = outer_product.float()  
